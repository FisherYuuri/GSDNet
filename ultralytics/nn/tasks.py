...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, OBB, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost,
                                    ...
                                    RepConv, ResNetLayer, RTDETRDecoder, Segment, CBAM, SFEBlock, SFEBlockN, SA)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, CBAM, SFEBlock, SFEBlockN, SA):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, SFEBlock, SFEBlockN):
...
def yaml_model_load(path):
    """Load a GSDNet model from a YAML file."""
    path = Path(path)

    unified_path = re.sub(r'(GSDNet)([ns])(.+)?$', r'\1\3', str(path))  # i.e. GSDNetn.yaml -> GSDNet.yaml
...
def guess_model_scale(model_path):
    ...
    with contextlib.suppress(AttributeError):
        import re
        return re.search(r'GSDNet+([nslmx])', Path(model_path).stem).group(1)  # n, s, m, l, or x
    return ''
